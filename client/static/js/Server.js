...
export default class Server {
    static getPowerSourcesBreakdown(ISOZoneId, callback) {
        const targetUrl =
            root_endpoint +
            "/zones/" +
            ISOZoneId +
        ...
        async.waterfall(
            [
                ...
                (rawData, cb) => {
                    rawData.json().then((data) => cb(data));
                },
            ],
            (err, data) => {
                // Sort data
                ...
                const elementsIndex = this.state.loadingStatuses.findIndex(
                    (elem) => elem.name == "fetching power source breakdown"
                );
                const newArray = [...this.state.loadingStatuses];

                newArray[elementsIndex].status = "OK!";
                console.log(newArray);
                this.setState({
                    loadingStatuses: newArray,
                });
    ...
    static getLoadsBreakdown(ISOZoneId, callback) {
        const targetUrl =
    ...
    static getLoadsForAllZones(callback) {
        const targetUrl =
